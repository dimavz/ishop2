...
//		exit();
        if (is_array($data)) {
            extract($data); //Функция extract извлекает данные из массива и создаёт переменные по названию ключей
        }

        //Вид это как правило динамическая часть (к примеру данные выбираются из БД),
        // а  макет (layout) по большей части статическая часть (шаблон в который вставляются данные (вид))
